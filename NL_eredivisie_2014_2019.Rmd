...
qplot(Season, HomeTeam, data=eredivisie, ylab="Team", xlab = "Season")
```
...
```{r monte_carlo, cache=TRUE, dependson="processed_data", cache.extra = "jags_model_description", results='hide'}
# Compiling the model
...
$$HomeGoals_{t,i,j} \sim \text{Poison}(\lambda_{\text{home},t,i,j})$$
$$AwayGoals_{t,i,j} \sim \text{Poison}(\lambda_{\text{away},t,i,j})$$
$$\log(\lambda_{\text{home},t,i,j}) = \text{baseline}_{\text{home},t} + \text{skill}_{t,i} - \text{skill}_{t,j}$$
$$\log(\lambda_{\text{away},t,i,j}) = \text{baseline}_{\text{away},t} + \text{skill}_{t,j} - \text{skill}_{t,i}$$
...
$$\text{baseline}_{\text{home},t} \sim \text{Normal}(0, 4^2)$$
$$\text{baseline}_{\text{away},t} \sim \text{Normal}(0, 4^2)$$

I set the prior distributions over the skill of all $n$ teams in season $t$ to:

$$\text{skill}_{t,1 \ldots n} \sim \text{Normal}(\text{skill}_{t-1,1 \ldots n}, \sigma_{\text{seasons}}^2)$$
$$\sigma_\text{seasons} \sim \text{Uniform}(0, 3)$$

and the priors over the skill of all $n$ teams in their $first$ season modeled as:

$$\text{skill}_{1,1 \ldots n} \sim \text{Normal}(\mu_\text{teams}, \sigma_{\text{teams}}^2)$$
...
```{r model_predictions, cache=TRUE, dependson="monte_carlo", tidy=FALSE}
n <- nrow(ms3)
...
hist(eredivisie$HomeGoals, breaks= (-1:max(eredivisie$HomeGoals)) + 0.5, xlim=c(-0.5, 10), main = "Distribution of the number of goals\nscored by a home team in a match",
    xlab = "")
```
...
#hist(m3_pred[ , "rand_home_goal"], xlim=c(-0.5, 10),
