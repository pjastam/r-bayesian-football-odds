...
knitr::opts_chunk$set(message=TRUE, warning=TRUE, fig.width=9,
...
```{r raw_data, cache = TRUE, cache.extra = file.info(list.files(path = "test", pattern = ".csv", full.names = TRUE))}
source(paste0("functions/Read_N",from_year,to_year,".R"))
```
...
eredivisie <- eredivisie %>%
  ...
  filter(!is.na(MatchResult)) # Creating a data frame d with only the complete match results

# Lists with the unique names of all teams and all seasons in the database
teams <- unique(c(eredivisie$HomeTeam, eredivisie$AwayTeam))
seasons <- unique(eredivisie$Season)

# A list for JAGS with the data from d where the strings are coded as integers
data_list <- list(HomeGoals = eredivisie$HomeGoals, AwayGoals = eredivisie$AwayGoals, 
                  HomeTeam = as.numeric(factor(eredivisie$HomeTeam, levels=teams)),
                  AwayTeam = as.numeric(factor(eredivisie$AwayTeam, levels=teams)),
                  Season = as.numeric(factor(eredivisie$Season, levels=seasons)),
                  n_teams = length(teams), n_games = nrow(eredivisie), 
...
```{r jags_model_description, cache=TRUE, tidy=FALSE}
m3_string <- "model {
for(i in 1:n_games) {
...
```{r monte_carlo, cache=TRUE, dependson="jags_model_description", results='hide'}
# Compiling the model
...
```{r team_skill_PSV_Ajax, fig.height=3, fig.width=7, results='hide'}
...
```{r model_predictions, tidy=FALSE}
n <- nrow(ms3)
